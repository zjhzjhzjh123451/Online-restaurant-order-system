		...
		// alert(value);
	 	if (dish_typeid==dishtypeid) {
	 		...
	 		$(".row.no-gutters").prepend(
	 			...
	 			"<a class=\"portfolio-box\" data-toggle=\"modal\" data-target=\"#"+this_dish.fields.name+"_floating\">"+
...
function updating_floatingwindows(items) {
	...
	for (var i = 0; i < dishes.length; i++) {
		...
		$("#floating_windows").prepend(
				"<div class=\"modal fade\" id=\"" +this_dish.fields.name+"_floating\" tabindex=\"-1\" role=\"dialog\" aria-labelledby=\"exampleModalCenterTitle\" aria-hidden=\"true\">"+
				...
				"</div>"+
				"<input type=\"text\" class=\"form-control\" value=\"0\" aria-label=\"Username\" aria-describedby=\"basic-addon1\" id=\""+this_dish.fields.name+"_quntity\">"+
				"</div>"+
...
function update_cart(number,name,price){
	...
	if (document.getElementById(name)) {
		...
		"<input name=\"dish[]\" value=\""+name+"\">"+
		"<input name=\"num[]\" value=\""+number+"\">"+
		"</div>"
		);
